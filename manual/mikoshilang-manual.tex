...
# With all features (Jupyter + visualization + signal processing)
pip install mikoshilang[all]
\end{lstlisting}

\textbf{Note:} On Ubuntu 24.04+ the virtual environment is required due to PEP 668 (externally-managed-environment).

\subsubsection{Windows (Anaconda — Recommended)}
...
\textbf{Supported on all platforms:} Windows, macOS, Linux, WSL. The Jupyter kernel is fully compatible with Anaconda (fixed in v3.6.9 with \texttt{nest\_asyncio} integration).

\subsubsection{Windows (WSL — Alternative)}
...
\subsubsection{macOS (Apple Silicon / Intel)}
...
# For M1/M2/M3 Macs: all dependencies are ARM64-compatible
\end{lstlisting}
...
\section{Knowledge Layer (v3.5.0)}

MikoshiLang v3.5.0 introduces a complete knowledge layer with deterministic fact retrieval from multiple sources. Unlike LLMs that answer from memory, MikoshiLang queries structured databases and returns provenance-tracked results.
...
\begin{enumerate}
    ...
    \item \textbf{Domain Packs} — PubChem, Crossref, OpenAlex, GeoNames, World Bank, AlphaFold, JHTDB
...
MikoshiLang includes 7 domain-specific knowledge sources:
...
\subsubsection{AlphaFold (Protein Structures)}

\begin{lstlisting}
PackSearch["alphafold", "hemoglobin"]
# => [{"id": "P69905", "label": "Hemoglobin subunit alpha", 
#      "license": "CC BY 4.0"}]

PackValue["alphafold", "P69905", "Confidence"]
# => {"value": 93.2}

PackValue["alphafold", "P69905", "Structure"]
# => {"value": "https://alphafold.ebi.ac.uk/files/AF-P69905-F1..."}
\end{lstlisting}

\textbf{Coverage:} 200M+ protein structures \\
\textbf{Properties:} Structure, Confidence, Sequence, Organism, Gene, Function

\subsubsection{JHTDB (Turbulence Simulations)}

\begin{lstlisting}
PackSearch["jhtdb", "channel"]
# => [{"id": "channel5200", "label": "Channel Flow (Re=5200)", 
#      "license": "Free for research/education"}]

PackValue["jhtdb", "channel5200", "ReynoldsNumber"]
# => {"value": 5200}

PackValue["jhtdb", "isotropic1024coarse", "Resolution"]
# => {"value": "1024^3"}
\end{lstlisting}

\textbf{Coverage:} 6 DNS datasets \\
\textbf{Properties:} Name, Resolution, ReynoldsNumber, TimeRange, Fields \\
\textbf{Datasets:} Isotropic turbulence, channel flow (Re=1000, Re=5200), mixing layer, MHD turbulence, transitional boundary layer
...
MikoshiLang provides a custom Jupyter kernel for interactive Wolfram-style notebooks with LaTeX rendering and inline plots.
...
\begin{itemize}
    \item \textbf{Linux:} ✓ Fully supported
    \item \textbf{macOS:} ✓ Fully supported (Intel \& Apple Silicon)
    \item \textbf{Windows (Anaconda):} ✓ Fully supported (fixed in v3.6.9)
    \item \textbf{Windows (WSL):} ✓ Fully supported
\end{itemize}

\textbf{Anaconda Compatibility:} As of v3.6.9, the MikoshiLang kernel is fully compatible with Anaconda on all platforms. The kernel applies \texttt{nest\_asyncio.apply()} before launch to fix Windows ProactorEventLoop conflicts.

\subsubsection{Usage}

\begin{enumerate}
    \item Create a new notebook in Jupyter
    \item Select \textbf{MikoshiLang} from the kernel dropdown
    \item Use Wolfram-style syntax directly:
\end{enumerate}

\begin{lstlisting}
In[1]:= x^2 + 2*x + 1
Out[1]= x^2 + 2*x + 1  (rendered as LaTeX)
...
# Reinstall kernel
python -m mikoshilang.jupyter.install --user

# Check Python environment matches Jupyter
which python
which jupyter
\end{lstlisting}
